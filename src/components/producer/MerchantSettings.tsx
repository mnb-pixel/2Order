...
import { vatRateFor } from '../../lib/pricing';
import { ShieldCheck, CreditCard, Clock, CheckCircle2, Kanban, Building2, Lock } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-swiss space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 text-xs font-mono">
          ...
          <div className="bg-stone-50 p-3.5 rounded-xl border border-stone-200">
            ...
            <span className="text-stone-500 text-[11px]">{(vatRateFor(currentProducer.country) * 100).toFixed(1)}% {currentProducer.country} Regelsatz</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-swiss space-y-4">
        ...
        <p className="text-xs text-stone-600 leading-relaxed">
          Wird beim Wechsel in dieses Produzenten-Portal abgefragt. Einfacher Zugriffsschutz für ein gemeinsam genutztes Gerät — kein vollständiges Login-System.
        </p>
        <div>
          <label className="font-mono text-stone-600 block mb-1 font-bold">Zugriffscode (4–6 Ziffern)</label>
          <input
            type="text"
            inputMode="numeric"
            pattern="\d{4,6}"
            defaultValue={currentProducer.portalPin}
            onBlur={(e) => {
              const val = e.target.value.replace(/\D/g, '').slice(0, 6);
              if (/^\d{4,6}$/.test(val)) updateProducer(currentProducer.id, { portalPin: val });
              else e.target.value = currentProducer.portalPin;
            }}
            className="w-32 px-3 py-2 border border-stone-300 rounded-lg font-mono font-bold text-xs tracking-widest"
          />
        </div>
      </div>
